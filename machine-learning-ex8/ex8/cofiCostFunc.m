...
Theta_reg = (lambda / 2) * sum(sum(Theta .^ 2, 2));
X_reg = (lambda / 2) * sum(sum(X .^ 2, 2));
X_grad_reg =  lambda * X;
Theta_grad_reg =  lambda * Theta;

errors = ((X * Theta') - Y) .* R;
J = (1 / 2) * sum(sum(errors .^2)); # J is a real number and not a vector
J += Theta_reg + X_reg;
X_grad =  errors * Theta;
X_grad += X_grad_reg;
Theta_grad = errors' * X;
...
assert(size(X_grad) == size(X), '');
assert(size(Theta_grad) == size(Theta), '');
