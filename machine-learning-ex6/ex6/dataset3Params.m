...
% values = [0.1, 0.3];  % debug only, remove. Optimized choices for test data.
...
for c=1:l,
    for sigma=1:l,
      ...
      fprintf('\nEvaluation %d of %d with C = %.3f and sigma = %.3f has error %.3f', 
        (((c - 1) * l) + sigma), l^2, testC, testSigma, testError);
